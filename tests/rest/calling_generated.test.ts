...
describe('calling wire (generated)', () => {
  it('calling_aiHold success', async () => {
    await client.calling.aiHold('x');
    const last = await mock.last();
    expect(last.method).toBe('POST');
    expect(last.matched_route).toBe('calling.call-commands');
  });

  it('calling_aiHold error', async () => {
    await mock.pushScenario('calling.call-commands', 500, { error: 'x' });
    await expect(client.calling.aiHold('x')).rejects.toThrow(RestError);
    const last = await mock.last();
