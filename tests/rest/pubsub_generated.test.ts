...
describe('pubsub wire (generated)', () => {
  it('pubsub_createToken success', async () => {
    await client.pubsub.createToken(1, {});
    const last = await mock.last();
    expect(last.method).toBe('POST');
    expect(last.matched_route).toBe('pubsub.create_token');
  });

  it('pubsub_createToken error', async () => {
    await mock.pushScenario('pubsub.create_token', 500, { error: 'x' });
    await expect(client.pubsub.createToken(1, {})).rejects.toThrow(RestError);
    const last = await mock.last();
