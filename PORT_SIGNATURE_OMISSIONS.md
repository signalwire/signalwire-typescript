...
signalwire.core.mixins.ai_config_mixin.AIConfigMixin.add_language: UNDER REVIEW (2026-07-30) — NOT idiom. The bag now UNFOLDS (methodOptionsBagMembers resolves the named `LanguageConfig` interface), and doing so revealed three REAL divergences that the previous "the unfold cannot explode it" rationale was hiding: (1) `voice` is REQUIRED in the reference and optional in `LanguageConfig`; (2) the reference's `speech_fillers: list[str]` is spelled `fillers?: Record<string, string[]>` — a different shape, not a rename; (3) the reference's `function_fillers: list[str]` is spelled `functionFillers?: Record<string, Record<string, string[]>>` — likewise. `speechModel` has no reference twin. Closing these means changing the port's PUBLIC `LanguageConfig` interface (SemVer-breaking), which is an owner call, not an enumerator fold — itemised for a ruling rather than re-excused as idiom. Reference source: signalwire/core/mixins/ai_config_mixin.py:87-96.
