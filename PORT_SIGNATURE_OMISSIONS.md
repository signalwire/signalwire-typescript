...
signalwire.core.mixins.ai_config_mixin.AIConfigMixin.add_language: ts-named-options-type: addLanguage(config: LanguageConfig) — the options bag is a NAMED exported interface, not an inline type literal, so the signature enumerator's options-object unfold (which reads an inline `ts.TypeLiteralNode`) cannot explode it. Members map 1:1 onto the reference's keyword set and each is read individually onto the wire (config.name/code/voice/...). This is the named-class-type vocabulary question tracked as the `stricter/superset` bucket, not an options-object idiom.
signalwire.core.mixins.ai_config_mixin.AIConfigMixin.add_pronunciation: ts-named-options-type: addPronunciation(rule: PronunciationRule) — same as add_language: the bag is a NAMED interface rather than an inline type literal, so the unfold cannot reach its members. `rule.replace`/`rule.with`/`rule.ignoreCase` are read individually onto the wire, matching the reference's keyword set.
signalwire.core.swml_handler.AIVerbHandler.build_config: ts-named-options-type: buildConfig(opts: AIVerbBuildOptions = {}) — the bag is a NAMED exported interface (destructured in the body), not an inline type literal, so the options-object unfold does not fire. Same AI verb config emitted.
