...
class MyAgent extends AgentBase {
  ...
  async onSummary(summary: Record<string, unknown> | null, rawData: Record<string, unknown>): Promise<void> {
    if (summary) {
...
The SDK extracts the summary from the post-prompt payload by checking `body.summary`, then `body.post_prompt_data.parsed[0]`, then parsing `body.post_prompt_data.raw` as JSON.
...
| `SWML_PROXY_URL_BASE` | *(none)* | External-facing base URL for webhook URLs (e.g., `https://agents.example.com`). Takes highest priority for proxy detection. |
| `SWML_PROXY_DEBUG` | `"false"` | Set to `"true"` to enable verbose logging of proxy/header detection. |
| `SWML_CORS_ORIGINS` | `"*"` | Comma-separated list of allowed CORS origins. Defaults to allow all. |
